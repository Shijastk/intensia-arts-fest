...
const getGroupResults = (teams: any[]) => {
  const results = teams.filter(t => (t.rank && t.rank > 0) || (t.points && t.points > 0)).map(team => {
    return {
      name: team.teamName, // For group items, team name is the primary display
    ...
    <div className="flex items-center gap-3">
      <div
        className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black ${
          data.rank === 1 ? 'bg-yellow-200 text-yellow-800' : 
          data.rank === 2 ? 'bg-slate-200 text-slate-600' : 
          data.rank === 3 ? 'bg-orange-200 text-orange-800' : 'bg-blue-100 text-blue-600'
        }`}
      >
        {data.rank !== 999 ? data.rank : '-'}
      </div>
      <div>
        <p className="text-sm font-black uppercase">{data.name}</p>
        <p className="text-[10px] font-bold uppercase text-slate-400">{data.teamName}</p>
      </div>
    </div>
...
export const ResultsPage: React.FC<ResultsPageProps> = ({ programs }) => {
  ...
  const completedPrograms = useMemo(
    () => programs.filter(p => p.status === ProgramStatus.COMPLETED && p.isResultPublished),
    [programs]
  );
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <nav className="sticky top-0 bg-white border-b border-slate-100 z-50">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          <Link to="/" className="font-black uppercase">Intensia</Link>
          <Link to="/" className="text-xs font-bold uppercase text-slate-500">Back</Link>
        </div>
      </nav>

      <div className="py-14">
        <div className="max-w-7xl mx-auto px-6">
          <h1 className="text-4xl font-black uppercase mb-10">Published Results</h1>
            ...
            <div className="md:w-64">
              ...
              >
                ...
                {availableZones.map(zone => (
                  <option key={zone} value={zone}>{zone === 'General' ? 'General' : `Zone ${zone}`}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          {filteredPrograms.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPrograms.map(prog => (
                <div key={prog.id} className="bg-white rounded-3xl p-4 border border-slate-200 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                  <div className="h-36 bg-slate-100 rounded-[1.5rem] p-4 relative">
                    <div className="flex gap-2">
                      <span className="inline-block bg-white px-3 py-1 rounded-lg text-[10px] font-black uppercase">{prog.category}</span>
                      <span className="inline-block bg-emerald-500 text-white px-3 py-1 rounded-lg text-[10px] font-black uppercase">Zone {extractZone(prog.category)}</span>
                    </div>
                    <div className="absolute bottom-4 left-4 right-4">
                      <h3 className="text-xl font-black uppercase">{prog.name}</h3>
                    </div>
                  </div>
                  <div className="p-4 space-y-3">
                    {prog.isGroup
                      ? getGroupResults(prog.teams).map((r, index) => <ResultRow key={index} data={r} index={index} />)
                      : getIndividualResults(prog.teams).map((r, index) => <ResultRow key={index} data={r} index={index} />)
                    }
                  </div>
                </div>
              ))}
            ...
            <div className="text-center py-20">
              <p className="text-2xl font-black uppercase text-slate-300 mb-2">No Results Found</p>
              <p className="text-sm font-bold text-slate-400">Try adjusting your search or filter criteria</p>
            </div>
          )}
        </div>
      </div>
